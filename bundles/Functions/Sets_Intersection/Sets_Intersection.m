...
#import "Sets_Intersection.h"


@implementation Sets_Intersection
...
-(void)perform{
	...
	for(QuinceObject * qa in [a valueForKey:@"subObjects"]){
		BOOL flag = NO;
		for(QuinceObject * qb in [b valueForKey:@"subObjects"]){
		
			if ([qa isEqualTo:qb])
				flag = YES;
		}
		if(flag == YES)
			[[mom controller]addSubObjectWithController:[document controllerForCopyOfQuinceObjectController:[qa controller] inPool:NO] withUpdate:NO];	
	}
